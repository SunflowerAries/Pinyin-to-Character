...
def viterbi(hmm_params, observations, path_num=6, log=False, min_prob=3.14e-200):
    ...
    for state in cur_states:
        if log:
            __score   = np.log(max(hmm_params.start(state), min_prob)) + \
                np.log(max(hmm_params.emission(state, cur_obs), min_prob))
        else:
        ...
        for y in cur_states:
            ...
            for y0 in prev_states:  # from y0(t-1) to y(t)
                for item in V[0][y0]:
                    if log:
                        _s = item.score + \
                            np.log(max(hmm_params.transition(y0, y), min_prob)) + \
                            np.log(max(hmm_params.emission(y, cur_obs), min_prob))
                    else:
                        _s = item.score * \
